const items = [
  {
    ...
    body: 'ILWU Local 23 invites all members, families, and supporters to the annual Bloody Thursday commemoration at Spanaway Lake Park. We gather to honor the six workers who gave their lives in 1934 — the sacrifice that built everything we have today. Speakers, food, and fellowship. Bring the family.',
  },
  {
    day: '18', month: 'JUN', year: '2026',
    cat: 'Meeting',
    title: 'Stop Work Meeting — All Members Required',
    body: 'A Stop Work Meeting has been called for June 18. All Local 23 members are required to attend. Work at the Port of Tacoma will stop for the duration of the meeting. Location and time posted at the hall. Check the dispatch board for updates.',
  },
  {
    day: '16', month: 'JUN', year: '2026',
    cat: 'Meeting',
  ...
  },
  {
    ...
    body: 'It is with great sorrow that Local 23 announces the passing of our brother David Bartholomew. A dedicated member who served this local with pride and commitment. He is deeply missed by his family, his crew, and everyone who worked alongside him.',
  },
  {
    day: '14', month: 'MAY', year: '2026',
    cat: 'Memorial',
    title: 'In Memoriam: Marco Hernando',
    body: 'Local 23 mourns the loss of our brother Marco Hernando. A proud union man who embodied what it means to look out for your fellow worker. His family, his gang, and this local are diminished by his absence.',
  },
]

const catStyle = {
  Event:    'bg-ilwu-gold text-ilwu-navy',
  Meeting:  'bg-ilwu-navy text-white',
  Alert:    'bg-red-600 text-white',
  Action:   'bg-blue-600/80 text-white',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {items.map((item, i) => (
              <article
                key={i}
                className={`bg-white border border-ilwu-border hover:border-ilwu-navy/30 transition-colors p-6 flex flex-col gap-4 shadow-sm ${item.cat === 'Memorial' ? 'opacity-90' : ''}`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-baseline gap-3">
                    <span className="font-display text-ilwu-navy text-5xl leading-none">{item.day}</span>
                    <div className="flex flex-col">
                      <span className="font-mono text-ilwu-navy/50 text-sm leading-tight uppercase tracking-widest">{item.month}</span>
                      <span className="font-mono text-ilwu-navy/30 text-xs leading-tight">{item.year}</span>
                    </div>
                  </div>
                  <span className={`text-[10px] font-body font-bold uppercase tracking-wider px-3 py-1 flex-shrink-0 ${catStyle[item.cat]}`}>
                    {item.cat}
                  </span>
                </div>
